    ...
    <div className="py-2 mb-8 border-2 rounded-lg h-1/2">
      ...
      <div className="flex justify-between items-center px-2 xl:px-2 lg:px-0">
        <div className="flex px-1 md:px-1 py-4 items-center xl:px-6 lg:px-4">
          <img src={profile_img ? profile_img : Ellipse4} alt="Profile" className="w-12 h-12 rounded-full object-cover object-top" />
          <div className="ml-2 lg:ml-4 md:ml-2">
            <div className="md:text-lg lg:text-xl font-semibold">{name}</div>
        ...
        <div className="flex px-0 py-4 items-center md:px-0 xl:px-2 lg:px-0 ">
          <button className="text-primary text-base lg:text-lg xl:text-xl mr-1 xl:mr-4 lg:mr-2 md:mr-1" onClick={() => handleFollow(follow ? "unfollow" : "follow")}>{follow ? "Unfollow" : "Follow"}</button>
      ...
      <div className="flex-col ">
        ...
        <img src={image} alt={description} className="object-cover object-top mt-2 w-full h-96 me-auto" />
      </div>
      <div className="flex justify-between mt-3 mb-2 items-center mx-9">
        <div className="flex items-center space-x-2 cursor-pointer" onClick={() => handleLike(like ? "unlike" : "like")}>
          ...
          />
          <span className="home-like-share-saved">{likecount} {like ? "Likes" : "Like"} </span>
        </div>
        ...
        <div className="flex items-center space-x-2 cursor-pointer" onClick={() => handlesaved(saved ? "deleteSave" : "save")}>
          <FontAwesomeIcon
            icon={saved ? solidBookmark : regularBookmark}
            className="text-gray-600"
          />
          {/* <img src={iconamoonbookmarkthin} alt="Save" className="w-6 h-6" /> */}
          <span className="home-like-share-saved">{saved ? "Saved" : "Save"}</span>
        </div>
      </div>
    </div>
  );
...
const ArticleCard = ({ title, description, author, image, user_id, follow, article_id, likecount, like, saved }) => {
