...
const SavedPostImage = ({ post, dataFetch, userData }) => {
    ...
    useEffect(() => {
        ...
        image.onload = () => {
            ...
            height > width || width === height ? setIsPortrait(true) : setIsPortrait(false);
        }
    }, [post.image])
        ...
        <div className="py-2 mb-10 border-2 rounded-lg mt-5" >
            ...
            <div className="flex-col">
                ...
                <img
                    src={post.image}
                    alt={post.title}
                    className={`${isPortrait ? 'px-3 object-contain mt-2 w-full h-96' : 'xss:px-3 sm:px-6 xss:object-contain xl:object-fill mt-2 w-full h-96 me-auto'} `}
                />
            </div>
            <div className="flex justify-between mt-3 mb-2 items-center mx-9">
                <div className="flex items-center space-x-2 cursor-pointer" onClick={() => handleLike(post.am_i_liked ? "unlike" : "like")}>
                    <FontAwesomeIcon
